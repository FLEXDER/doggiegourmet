...
/* ============================================================
   MASTER DASHBOARD — tabs entre Reportes, Catálogo e Inventario
   ============================================================ */
function MasterDashboard({ session, onLogout }) {
  const [view, setView] = useS3('reports'); // 'reports' | 'catalog' | 'inventory'
  ...
  const titles = {
    reports: { h1: <>Reportes <em>de inventario</em></>, sub: 'Resumen de todos los reportes enviados por los puntos de venta. Cambia el estatus conforme avanzas en la operación.' },
    catalog: { h1: <>Catálogo <em>de productos</em></>, sub: 'Administra los productos disponibles. Estos productos aparecen como sugerencias en los reportes y son la base del control de inventario.' },
    inventory: { h1: <>Control <em>de inventario</em></>, sub: 'Lleva el inventario consignado en cada punto de venta. Registra entregas, ventas y ajustes para mantener el saldo actualizado.' }
  };
...
/* ============================================================
   REPORTS VIEW — la vista de reportes (lo que ya existía)
   ============================================================ */
...
/* ============================================================
   CATALOG VIEW — gestión de productos
   ============================================================ */
function CatalogView() {
  const [products, setProducts] = useS3([]);
  const [loading, setLoading] = useS3(true);
  const [categoryFilter, setCategoryFilter] = useS3('all');
  const [search, setSearch] = useS3('');
  const [editing, setEditing] = useS3(null); // null = modal cerrado, {} = nuevo, {id, ...} = editar
  ...
  const handleSaved = (updatedOrNull) => {
    setEditing(null);
    refresh();
  };
...
function CatalogProductCard({ product, onClick }) {
  ...
  const cat = categoryColors[product.category] || { bg: 'var(--brown-soft)', label: product.category };
...
function ProductEditModal({ product, onClose, onSaved }) {
  ...
  const save = async () => {
    ...
    if (isNew) {
      ...
      result = await sb.from('products').update(payload).eq('id', product.id).select().single();
    }
  ...
  const remove = async () => {
    ...
    const { error: delErr } = await sb.from('products').delete().eq('id', product.id);
    if (delErr) {
...
/* ============================================================
   INVENTORY VIEW — control de inventario por POS
   ============================================================ */
const POS_LIST = [
...
function InventoryView() {
  const [selectedPos, setSelectedPos] = useS3('vitalpets');
  const [inventory, setInventory] = useS3([]);
  const [movements, setMovements] = useS3([]);
  const [products, setProducts] = useS3([]);
  const [loading, setLoading] = useS3(true);
  const [movementModal, setMovementModal] = useS3(null); // null | 'entrada' | 'venta' | 'ajuste'

  const refresh = async () => {
    setLoading(true);
    // Cargar saldos de la vista pos_inventory_summary
    const { data: summaryData, error: summaryErr } = await sb
    ...
    // Cargar productos activos para los modales
  ...
  const currentPos = POS_LIST.find(p => p.id === selectedPos);
      ...
      <div className="inv2-master-hero" style={{ paddingTop: 32, paddingBottom: 36 }}>
        <div className="container">
            ...
            {POS_LIST.map((p) =>
              <button
                ...
                style={{
                  ...
                  border: selectedPos === p.id ? '1.5px solid var(--green)' : '1.5px solid rgba(250, 247, 242, 0.18)',
                  padding: '14px 24px',
                  borderRadius: 100,
                  fontFamily: 'inherit',
                  fontSize: 14,
          ...
          <div className="inv2-master-stats">
            ...
            <Stat label="Valor consignado" value={`$${stats.totalValue.toLocaleString('es-MX', { maximumFractionDigits: 0 })}`} accent="green" />
...
/* ============================================================
   INVENTORY TABLE — saldos actuales
   ============================================================ */
...
/* ============================================================
   MOVEMENTS TABLE — histórico de movimientos
   ============================================================ */
function MovementsTable({ movements }) {
  ...
  return (
    <div style={{
      ...
      {movements.map((m, idx) => {
        const cfg = typeConfig[m.movement_type] || { label: m.movement_type, color: 'var(--brown-soft)', sign: '' };
...
/* ============================================================
   MOVEMENT MODAL — capturar entrada / venta / ajuste
   ============================================================ */
function MovementModal({ type, posId, posName, products, inventory, onClose, onSaved }) {
  ...
  const [adjustDirection, setAdjustDirection] = useS3('add'); // 'add' | 'subtract'
  const [date, setDate] = useS3(new Date().toISOString().slice(0, 10));
  const [rows, setRows] = useS3([
    ...
    <div
      ...
      <div
        ...
        <div style={{ padding: '20px 28px' }}>
          ...
          {/* Rows */}
          {rows.map((row, idx) => {
            const rowSubtotal = Number(row.quantity || 0) * Number(row.unit_price || 0);
            return (
              <div
                key={row.id}
                style={{
